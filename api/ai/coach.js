...
const systemPrompt = [
  ...
  "categoryCopy.action과 missionCopy의 title, action만 더 친절하고 구체적으로 다듬는다.",
...
function mergeEnhancement(baseReport, enhancement) {
  ...
  return {
    ...
    categoryPlans: baseReport.categoryPlans.map((plan) => {
      ...
      return copy ? { ...plan, action: copy.action } : plan;
    }),
    missions: baseReport.missions.map((mission) => {
      const copy = missionCopy.get(mission.id);
      return copy ? { ...mission, title: copy.title, action: copy.action } : mission;
    }),
  };
}
  ...
  try {
    ...
    const promptPayload = {
      ...
      baseReport: {
        ...
        categoryPlans: payload.baseReport.categoryPlans.slice(0, 3).map((plan) => ({
          category: plan.category,
          status: plan.status,
          currentAmount: plan.currentAmount,
          plannedAmount: plan.plannedAmount,
          previousRatio: plan.previousRatio,
          currentRatio: plan.currentRatio,
          action: plan.action,
        })),
        missions: payload.baseReport.missions.slice(0, 2).map((mission) => ({
          id: mission.id,
          title: mission.title,
          expectedSaving: mission.expectedSaving,
