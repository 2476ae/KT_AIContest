...
- AI는 챗봇보다 `오늘의 조정`, `이번 주 미션`, `구독 점검`처럼 행동 중심으로 보인다.
...
- `npm run verify`: Vitest 6개 파일, 24개 테스트와 Vite production build 통과
...
8. 코치 화면에서 오늘의 조정, 미션, 구독 점검, AI 상태 카드가 비어 있지 않은지 확인한다.
...
- 소비 유형 자동 분류
- 월간 소비 캘린더와 과소비일, 무지출일, 구독 결제일 표시
- 목표 소비액과 목표 저축액 기반 하루 예산 계산
- 오늘의 조정, 이번 주 미션, 구독 점검 제안
- 금융 인증정보 미수집 안내
...
1. 캘린더 화면으로 이동한다.
2. 초과, 구독, 무지출 필터를 보여준다.
3. 특정 날짜를 선택해 거래 목록과 분류를 보여준다.

말할 내용:

```text
날짜별 소비 상태를 색으로 구분하고, 구독 결제일과 과소비일을 빠르게 찾을 수 있습니다.
```
...
1. 코치 화면으로 이동한다.
2. 오늘의 조정, 이번 주 미션, 구독 점검, AI 상태 카드를 보여준다.
...
- [ ] Vitest 6개 파일, 24개 테스트 통과
- [ ] GitHub Actions 최신 run 성공
- [ ] `https://2476ae.github.io/KT_AIContest/` 접속 성공
- [ ] PC 브라우저에서 가로 스크롤 없음
