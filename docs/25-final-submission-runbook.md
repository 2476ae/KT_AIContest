...
- `npm run verify`: Vitest 13개 파일, 68개 테스트와 Vite production build 통과
- `npm run build:github-pages`: `/KT_AIContest/` base path 기준 production build 통과
- `npm run test:e2e`: Playwright 24개 시나리오로 심사위원 핵심 흐름과 금융 인증정보 미수집 안내 확인
...
11. 배포 환경에서 `AI 분석 업데이트`를 눌러 OpenAI 프록시 연결 또는 fallback 안내가 정상 표시되는지 확인한다.
...
- [ ] `npm run test:e2e` 통과
- [ ] Vitest 13개 파일, 68개 테스트 통과
- [ ] GitHub Actions 최신 run 성공
- [ ] `https://kt-ai-contest.vercel.app/` 접속 성공
- [ ] 배포 환경의 `AI 분석 업데이트`가 OpenAI 프록시 응답 또는 fallback 안내를 표시
- [ ] PC 브라우저에서 가로 스크롤 없음
