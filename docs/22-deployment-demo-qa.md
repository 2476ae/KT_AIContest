...
2026-07-10 기준으로 아래 명령을 통과해야 한다.
...
1. 홈 화면에서 서비스명, 샘플 데이터 CTA, 금융 인증 없이 체험 가능한 흐름을 확인한다.
2. 홈 또는 설정 화면에서 `샘플 데이터 불러오기`를 눌러 35건 샘플 데이터를 복원한다.
3. 홈 화면에서 월 목표 금액, 진행률, 오늘 권장 한도, 구독 지출을 확인한다.
4. 캘린더 화면에서 과소비일, 정기 결제일, 선택일 상세 거래를 확인한다.
5. 목표 화면에서 목표 소비액을 바꾸고 `목표 저장`을 누른다.
6. 추가 화면에서 거래를 직접 입력하고 저장 성공 메시지를 확인한다.
7. 캘린더 화면에서 방금 입력한 거래가 선택일 상세에 반영되는지 확인한다.
8. 코치 화면에서 오늘의 소비 가이드, 미션, 정기 결제 점검, AI 상태를 확인한다.
9. 설정 화면에서 CSV 내보내기와 초기화/샘플 복원을 확인한다.
...
- 직접 거래 추가
- 코치 화면 AI 상태 카드 표시
- Vercel 환경변수 설정 후 `AI 분석 업데이트` 버튼이 OpenAI 프록시 응답 또는 fallback 안내를 표시
- 상단 방패 버튼과 설정 화면에서 금융 인증정보 미수집, 샘플·직접 입력·CSV 전용 데모 안내 표시
- CSV 내보내기
