...
2026-06-30 기준 저장소 이름은 `2476ae/KT_AIContest`이고, Vercel Production URL은 `https://kt-ai-contest.vercel.app/`이다. GitHub Pages는 백업 URL로 유지한다.
...
- `OPENAI_MAX_OUTPUT_TOKENS`: 선택 사항, 권장값 `320`
- `OPENAI_TIMEOUT_MS`: 선택 사항, 기본값 `7500`, 최대 `8000`
- `AI_RATE_LIMIT_ENABLED`: production 기본 활성화, 관리자 데모에서만 필요 시 `false`
- `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`: Vercel Marketplace의 Upstash Redis 연결 시 자동 주입되는 서버 전용 값
- `AI_RATE_LIMIT_KEY_SECRET`: 호출자 주소 HMAC용 임의의 서버 전용 문자열
- `AI_RATE_LIMIT_STORE_TIMEOUT_MS`: 선택 사항, 기본값 `1200`, 최대 `2000`
- `AI_ALLOWED_ORIGINS`: `https://kt-ai-contest.vercel.app,http://localhost:5173`
- `VITE_AI_PROXY_TIMEOUT_MS`: 선택 사항, 기본값이자 최대값 `8000`

Vercel 전체 배포에서는 프론트와 `/api/ai/*` 프록시가 같은 도메인에 있으므로 `VITE_AI_PROXY_BASE_URL`을 비워둘 수 있다. GitHub Pages 프론트에서 이 Vercel 프록시를 호출하려면 GitHub repository Variables에 다음 값을 추가한다.
...
현재 실제로 활성화와 QA가 확인된 제출 URL은 Vercel이다.

```text
https://kt-ai-contest.vercel.app/
```

GitHub Pages와 Netlify 설정도 유지되어 있으므로 대체 URL이 필요하면 같은 저장소를 연결해 사용할 수 있다. 다만 제출 직전에는 실제 제출 URL 하나를 기준으로 `npm run verify`, production build, Vercel 최신 Production deployment, 모바일 접속을 다시 확인한다.
