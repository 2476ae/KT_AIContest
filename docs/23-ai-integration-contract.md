...
- AI 코치 리포트는 `categoryPlans`로 분야별 소비 계획 카드를 최대 3개 제공한다. 현재 달 거래와 `previousMonthTransactions`의 지난달 분야별 비중을 비교해 늘어난 분야만 조정 후보로 삼고, 긴 문장은 provider 검증 단계에서 잘라 카드 UI를 보호한다.
