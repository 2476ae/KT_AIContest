...
2026-07-15 기준 `npm run verify`는 Vitest 12개 파일, 63개 테스트와 Vite production build를 통과합니다. GitHub Pages 백업 번들은 `npm run build:github-pages`로 확인합니다. `npm run test:e2e`는 데스크톱·모바일 18개 시나리오를 2개 워커로 검증하며, 전체 로컬 게이트는 `npm run verify:full`로 확인합니다.
...
- `VITE_AI_PROXY_TIMEOUT_MS`는 선택 사항이며 최대 `8000`ms로 제한됩니다. 응답이 늦으면 기본 분석으로 자동 복귀합니다.

서버 프록시 env:

- `OPENAI_API_KEY`
- `OPENAI_MODEL` 기본값 `gpt-4.1-mini`
- `OPENAI_MAX_OUTPUT_TOKENS` 권장값 `320`
- `OPENAI_TIMEOUT_MS` 기본값 `7500`ms, 최대 `8000`ms
- `AI_ALLOWED_ORIGINS` 예: `https://kt-ai-contest.vercel.app,http://localhost:5173`
- `AI_RATE_LIMIT_ENABLED`는 Vercel production에서 기본 활성화되며 env로 명시적으로 해제할 수 있습니다.

GitHub Pages를 제출 URL로 유지하는 경우 `VITE_AI_PROXY_BASE_URL`은 별도 Vercel/Node 프록시 배포 URL을 바라보게 설정합니다.

비용 보호를 위해 OpenAI 호출은 명시 행동에만 연결되어 있습니다. AI 코치 탭 진입만으로는 호출하지 않고, `AI 분석 업데이트` 버튼을 눌렀을 때만 요청합니다. 숫자와 추천 상태는 앱이 먼저 계산하고 OpenAI에는 최대 3개 분야와 2개 미션의 화면 문구 보완만 맡깁니다. 같은 분석의 진행 중 요청은 하나로 합치며 성공 결과는 캐시합니다. 직접 입력 화면도 `자동 분류` 상태로 저장할 때만 분류 API를 1회 호출하고, 샘플·CSV·금융 피드는 로컬 일괄 분류를 사용합니다.

서버 제한은 공개 운영의 1차 보호 수단입니다. 현재 메모리 카운터는 서버리스 인스턴스가 바뀌면 초기화될 수 있으므로 대규모 공개 운영에서는 Vercel/Redis 계열의 영속 rate limit을 추가해야 합니다. 프록시는 민감한 거래 내용 없이 성공·실패·시간 초과·응답 시간을 구조화 로그와 `Server-Timing` 헤더로 남깁니다.

배포 AI smoke check:

1. Vercel 환경변수에 `OPENAI_API_KEY`, `OPENAI_MODEL`, `AI_ALLOWED_ORIGINS`를 설정합니다.
2. 시크릿 창에서 `https://kt-ai-contest.vercel.app/`에 접속합니다.
3. 샘플 데이터를 불러온 뒤 AI 코치 화면에서 `AI 분석 업데이트`를 누릅니다.
4. AI 상태 카드가 `OpenAI 분석 완료`로 표시되는지 확인합니다.
5. 응답이 8초를 넘거나 실패해도 화면이 깨지지 않고 `기본 분석으로 전환` 안내와 앱 기본 분석이 보이는지 확인합니다.

예산 계산은 월 목표를 초과했더라도 바로 하루 권장 한도를 0원으로 고정하지 않습니다. 월수입과 저축 목표를 함께 고려해 `현실 조정 목표`와 `조정 후 예상 저축`을 계산하고, 남은 일수 기준 하루 한도를 다시 제안합니다.

AI 코치의 분야별 소비 계획은 현재 달만 보지 않고, 샘플 데이터의 이전 달 소비 비중을 함께 비교합니다. 예를 들어 지난달 식비 비중과 이번달 식비 비중을 비교해 늘어난 분야만 조정 후보로 잡고, 안정적인 분야는 유지/기록 중심으로 안내합니다.

## 프론트엔드 구현 범위

- 홈: 이번 달 지출, 목표 진행률, 하루 가능예산, 구독비, 최근 소비 표시
- 캘린더: 날짜별 지출, 과소비일, 적정 소비일, 정기 결제일, 기록 없음, 선택일 상세 표시
...
브라우저 상태는 버전이 포함된 저장 포맷으로 보관합니다. 기존 v1 상태는 자동 마이그레이션하고, 잘못된 날짜·금액·카테고리 거래는 복원 과정에서 제외합니다.

## 실시간 금융 데이터 반영

실제 금융 API가 연결되면 앱은 `money-routine:financial-transactions` 브라우저 이벤트로 들어온 지출 거래를 정규화해 반영합니다. 같은 `source + externalId` 거래가 다시 들어오면 중복 추가가 아니라 기존 거래 업데이트로 처리하고, 반영 즉시 홈, 캘린더, 알림, AI 코치 계산이 다시 갱신됩니다. 입금 거래, 잘못된 날짜/금액, 과도한 단일 거래, 200건 초과 배치는 자동 방어하며 상세 기준은 [실시간 금융 연동 안정성 보강](docs/28-financial-feed-hardening.md)에 정리했습니다.
...
1. 홈에서 `샘플 데이터 불러오기`를 눌러 35건의 소비 데이터를 복원합니다.
2. 홈에서 월 지출, 목표 진행률, 오늘 권장 한도, 구독 지출을 확인합니다.
3. 캘린더에서 과소비일, 정기 결제일, 선택일 상세 거래를 확인합니다.
4. 목표 화면에서 목표 소비액을 바꾸고 저장해 홈/AI 코치 재계산을 확인합니다.
5. 추가 화면에서 직접 소비를 하나 저장하고 캘린더 반영을 확인합니다.
6. AI 코치 화면에서 오늘의 소비 가이드, 미션, 정기 결제 점검, AI 상태를 확인합니다.
...
제출 버전은 샘플 데이터, 직접 입력, CSV 업로드, 브라우저 이벤트 기반 수신 경로만 사용합니다. 위 기능은 수상 후 고도화 단계에서 오픈뱅킹 또는 금융 마이데이터 연동으로 확장하는 로드맵으로 제시합니다.
