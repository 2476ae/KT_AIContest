...
2026-07-01 기준 `npm run verify`는 Vitest 9개 파일, 45개 테스트와 Vite production build를 통과합니다. GitHub Pages 백업 번들은 `npm run build:github-pages`로 확인합니다.
...
- `VITE_AI_PROXY_TIMEOUT_MS`는 선택 사항이며 기본값은 `45000`입니다.
...
비용 보호를 위해 OpenAI 호출은 명시 행동에만 연결되어 있습니다. AI 코치 탭 진입만으로는 호출하지 않고, `AI 분석 업데이트` 버튼을 눌렀을 때만 AI 코치 리포트를 요청합니다. 직접 입력 화면도 `자동 분류` 상태로 저장할 때만 분류 API를 호출하고, 카테고리를 직접 선택하면 외부 분류를 호출하지 않습니다.
...
5. AI 코치 화면에서 오늘의 소비 가이드, 미션, 정기 결제 점검, AI 상태를 확인합니다.
