...
2026-07-10 기준 `npm run verify`는 Vitest 9개 파일, 49개 테스트와 Vite production build를 통과합니다. GitHub Pages 백업 번들은 `npm run build:github-pages`로 확인합니다. 브라우저 smoke test는 `npm run test:e2e`, 전체 로컬 게이트는 `npm run verify:full`로 확인합니다.
...
- `OPENAI_TIMEOUT_MS` 기본값 `6500`ms, 최대 `8000`ms
...
비용 보호를 위해 OpenAI 호출은 명시 행동에만 연결되어 있습니다. AI 코치 탭 진입만으로는 호출하지 않고, `AI 분석 업데이트` 버튼을 눌렀을 때만 AI 코치 리포트를 요청합니다. 같은 분석의 진행 중 요청은 하나로 합칩니다. 직접 입력 화면도 `자동 분류` 상태로 저장할 때만 분류 API를 1회 호출하고, 샘플·CSV·금융 피드는 로컬 일괄 분류를 사용합니다.
