...
2026-07-15 기준 `npm run verify`는 Vitest 12개 파일, 63개 테스트와 Vite production build를 통과합니다. GitHub Pages 백업 번들은 `npm run build:github-pages`로 확인합니다. 브라우저 smoke test는 `npm run test:e2e`, 전체 로컬 게이트는 `npm run verify:full`로 확인합니다.
...
- `OPENAI_MAX_OUTPUT_TOKENS` 권장값 `420`
...
비용 보호를 위해 OpenAI 호출은 명시 행동에만 연결되어 있습니다. AI 코치 탭 진입만으로는 호출하지 않고, `AI 분석 업데이트` 버튼을 눌렀을 때만 요청합니다. 숫자와 추천 상태는 앱이 먼저 계산하고 OpenAI에는 최대 4개 분야와 3개 미션의 짧은 문장 보완만 맡깁니다. 같은 분석의 진행 중 요청은 하나로 합치며 성공 결과는 캐시합니다. 직접 입력 화면도 `자동 분류` 상태로 저장할 때만 분류 API를 1회 호출하고, 샘플·CSV·금융 피드는 로컬 일괄 분류를 사용합니다.
