...
- `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`을 설정하면 모든 서버리스 인스턴스가 같은 일일 호출 카운터를 사용합니다.
- `AI_RATE_LIMIT_KEY_SECRET`은 호출자 주소를 Redis key에 원문으로 남기지 않기 위한 서버 전용 난수입니다.
...
서버 제한은 공개 운영의 1차 보호 수단입니다. Upstash Redis 환경변수가 있으면 원자적 공용 일일 카운터를 사용하고, 저장소가 설정되지 않았거나 일시적으로 응답하지 않으면 인스턴스 메모리 제한으로 복귀합니다. Redis에는 호출자 주소 원문 대신 HMAC 식별자만 저장합니다. 프록시는 민감한 거래 내용 없이 성공·실패·시간 초과·응답 시간을 구조화 로그와 `Server-Timing` 헤더로 남깁니다.
